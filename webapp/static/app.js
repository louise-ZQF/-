...
async function runAiAnalysis(){
  ...
  try{
    ...
    if(d.funds && tagsEl){
      const nameMap={}; (state.data?.funds||[]).forEach(f=>{ nameMap[f.code]=f.name; });
      ...
      const tags=Object.entries(d.funds).map(([code,s])=>{
        ...
        if(s.sentiment==='看好'||s.sentiment==='强烈看好') bullish.push({code,label,s});
        else if(s.sentiment==='谨慎'||s.sentiment==='规避') bearish.push({code,label,s});
        else neutral.push({code,label,s});
