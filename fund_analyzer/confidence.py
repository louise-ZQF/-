...
def compute_confidence(fund: dict, rolling_scores: List[float] = None) -> dict:
    """计算基金评分的置信度。
    ...
    if inception:
        try:
            ...
            history_years = (as_of - inc_date).days / 365.25
    ...
    expected_days = fund.get("expected_days", 252)
    data_completeness = nav_days / max(expected_days, 1)
    ...
    confidence = history_score * 0.5 + data_completeness * 0.3 + metric_stability * 0.2
